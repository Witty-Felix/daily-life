...
export function createVirtueRequestHandler(options: VirtueServerOptions) {
  const resolveAccountId = options.resolveAccountId ?? ((authorization) => authorization?.startsWith("Bearer ") ? authorization.slice(7).trim() || null : null);
  ...
  return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    try {
      ...
      if (method === "POST" && id && url.pathname.endsWith("/permanent")) { const input = asObject(await body(req, maxBodyBytes)); api.permanentlyDelete(id, String(input.confirmation ?? "")); json(res, 204); return; }
