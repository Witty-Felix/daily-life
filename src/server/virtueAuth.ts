...
export function createSqliteVirtueAuth(options: SqliteAuthOptions = {}): SqliteAuthDatabase {
  ...
  const service: VirtueAuthService = {
    ...
    authenticateRecovery: (accountId, code, deviceName = "未命名设备") => { assertAccount(accountId); const row = db.prepare("SELECT * FROM virtue_auth_recovery WHERE account_id=?").get(accountId) as Row | undefined; if (!row || row.used_at || !same(String(row.code_hash), hashRecovery(accountId, code))) fail("invalid recovery code"); const timestamp = now().toISOString(); db.prepare("UPDATE virtue_auth_recovery SET used_at=? WHERE account_id=? AND used_at IS NULL").run(timestamp, accountId); db.prepare("UPDATE virtue_auth_accounts SET status='active' WHERE account_id=? AND status='invited'").run(accountId); const token = randomBytes(32).toString("base64url"); const id = randomBytes(16).toString("hex"); db.prepare("INSERT INTO virtue_auth_sessions(session_id,account_id,token_hash,device_name,created_at,last_used_at,revoked_at) VALUES(?,?,?,?,?,?,NULL)").run(id, accountId, hashToken(token), deviceName.trim() || "未命名设备", timestamp, timestamp); return { accountId, token, session: { id, accountId, deviceName: deviceName.trim() || "未命名设备", createdAt: timestamp, lastUsedAt: timestamp, revokedAt: null } }; },
